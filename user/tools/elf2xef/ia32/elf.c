...
int ProcessElf(int fdElf, int excludeSections[], int noExcludeSections) {
    ...
    if ((eHdr.e_type != ET_EXEC) || (eHdr.e_machine != EM_386) || (eHdr.e_phentsize != sizeof(Elf32_Phdr)) || (eHdr.e_phnum < 1)
            || (eHdr.e_phnum > 65536U / sizeof(Elf32_Phdr))) {
    ...
    for (e = 0, xefRel = 0, nRel = 0; e < eHdr.e_shnum; e++) {
        if ((sHdr[e].sh_type != SHT_REL) && (sHdr[e].sh_type != SHT_RELA))
            continue;
        if (!(sHdr[sHdr[e].sh_info].sh_flags & SHF_ALLOC))
            continue;
